...
void *process_request(void *vargp) 
{
    ...
    strcat(newrequest,"\r\n\r\n");
    printf("newrequest: %s",newrequest);
    printf("newrequest: %s",newrequest);
    ...
    if (clientfd < 0) {
        ...
        if (n != -1) {
            buf[n] = '\0';
            write(connfd, buf, n);
            format_log_entry(logstring, MAXLINE-1, &clientaddr, uri, n);
                      ...
                      struct sockaddr_in *sockaddr, char *uri, int size)
{
    ...
    printf("FORMATTED LOG STRING: %s",logstring);
}
